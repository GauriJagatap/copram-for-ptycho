...
%         pupil = ones([ROW COL]);
        pupil = zeros([ROW COL]);
        pupil(ceil(ROW/3):ceil(2*ROW/3),ceil(COL/3):ceil(2*COL/3)) = ones(length(ceil(ROW/3):ceil(2*ROW/3)));
%         pupil(ceil(ROW/4):ceil(3*ROW/4),ceil(COL/4):ceil(3*COL/4)) = ones(length(ceil(ROW/4):ceil(3*ROW/4)));
    case 'random'
        pupil = randi([0 1],ROW,COL);        
    otherwise
...
%         cind = VEC(repmat((1:COL)+(tr-1)*apShift+1, COL,1)); % no partial shifts allowed
        ...
        if subpixel
            switch lower(pupilType)
                case 'random'
%                     pupil_mask(:,:,count) = randi([0 1],ROW,COL); 
                    mat = rand(ROW,COL);
                    pupil_mask(:,:,count) = ceil(mat.*(mat>0.94));
                otherwise
